...
class Functions
{
    ...
    public static function searchQuantityDocumentsByDates(
        $array,
        $fieldSearch,
        $search,
        $format
        ...
        for ($i = 0; $i < count($array) - 1 && $flag == true; $i++) {
            $indexI = $i;
            if (
                date_format(date_create($search), $format) ==
                date_format(date_create($array[$i][$fieldSearch]), $format)
            ) {
                for (
                    $j = $i + 1;
                    $j < count($array) &&
                    date_format(date_create($search), $format) ==
                        date_format(
                            date_create($array[$j - 1][$fieldSearch]),
                            $format
                        );
                    $j++
                ) {
    ...
    public static function selectListMenus(
        $id,
        $selectChangeToNone = false,
        $id_menu_except = -1
    ) {
        $output = '';
        $datos = Menu::todos();
        ...
        $haySeleccionado =
            (is_array($seleccionado) == true and count($seleccionado) > 0);

        $output .=
            '<option value="-1">' .
            ($selectChangeToNone ? 'Ninguno' : '--Seleccione--') .
            '</option>';
    ...
    {
        $output = [];
        $directorios_en_db = array_map(function ($arr) {
            return $arr['ruta'];
        }, Modulos::todos());
        ...
        if (
            is_array($directorio_seleccionado) == true and
            count($directorio_seleccionado) > 0
        ) {
    ...
    {
        $output = $directorios_en_json = [];
        $directorios_en_db = array_map(function ($arr) {
            return $arr['nombre'];
        }, Modulos::todosActivos());
    ...
    public static function orgranigramaMenus($id = -1)
    {
        ...
        if (is_array($hijos) == true and count($hijos) > 0) {
            foreach ($hijos as $key => $hijo) {
                ...
                if (
                    is_array($existenHijos) == true and
                    count($existenHijos) > 0
                ) {
                    $sub_array['children'] = Functions::orgranigramaMenus(
                        $hijo['id']
                    );
                }
    ...
    public static function organigramaMenusWithModules(
        $id,
        $type = -1,
        $type_id = -1,
        $itsForSideMenu = false,
        $isChildren = false,
        &$countModules = 0
    ) {
        $output = [];
        ...
        if (is_array($hijos) == true and count($hijos) > 0) {
            foreach ($hijos as $key => $hijo) {
                ...
                $sub_array['children'] =
                    (is_array($existenHijos) == true and
                    count($existenHijos) > 0)
                        ? Functions::organigramaMenusWithModules(
                            $hijo['id'],
                            $type,
                            $type_id,
                            $itsForSideMenu,
                ...
                $modulosMenu = [];
                $existenModulos = Modulos::getByMenuId(
                    $hijo['id'],
                    $itsForSideMenu
                );
                if (
                    is_array($existenModulos) == true and
                    count($existenModulos) > 0
                ) {
                    $arr_permissions_by_type = [];
                    ...
                    switch ($type) {
                        case 0:
                            $arr_permissions_by_type = Permisos::getRolesGrupoPorRolID(
                                $type_id
                            );
                            break;
                        case 1:
                            $arr_permissions_by_type = Permisos::getPermisosPorUsuarioID(
                                $type_id
                            );
                            break;
                    }

                    $modulosInDB = array_map(function ($arr) {
                        return $arr['id_modulo'];
                    }, $arr_permissions_by_type);
