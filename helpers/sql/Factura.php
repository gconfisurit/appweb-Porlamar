...
class Factura extends Conectar
{
    ...
    public static function getHeaderById($numerod)
    {
        ...
        $sql = "SELECT numerod, fechae, codusua, codvend, descrip, Tasa AS tasa,
                ...
                FROM safact WHERE numerod = '$numerod'AND MtoTotal > 0";
    ...
    public static function getInvoiceReturns($fechai, $fechaf, $alm = [])
    {
        $i = 0;
        $cond = $depo = '';
        //LLAMAMOS A LA CONEXION QUE CORRESPONDA CUANDO ES SAINT: CONEXION2
        ...
        $sql =
            "SELECT CodItem AS coditem, Cantidad AS cantidad, esunid AS esunid FROM SAITEMFAC WHERE NumeroD IN (SELECT fa.NumeroR FROM SAFACT AS fa WHERE TipoFac= 'A' AND NumeroR IS NOT NULL " .
            $cond .
            " AND DATEADD(dd, 0, DATEDIFF(dd, 0, FechaE)) BETWEEN ? AND ?
               ...
               AND NumeroD NOT IN (SELECT Despachos_Det.Numerod FROM APPWEB_PORLAMAR.dbo.Despachos_Det) AND NumeroD NOT IN (SELECT numerof FROM sanota)) AND tipofac = 'B'";
