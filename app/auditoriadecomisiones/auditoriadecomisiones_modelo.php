		...
		if ($vendedor !== "") {
			$sql = "SELECT A.campo, A.antes, A.despu, B.usuario, B.fechah, E.nomper 
					FROM AUMCONFISUR.dbo.cambio_hist_comisiones AS A
						...
						INNER JOIN COMCONFISUR.dbo.periodo AS C ON B.cod_per = C.cod_per
						INNER JOIN COMCONFISUR.dbo.datos_edv AS D ON C.ci = D.ci
						INNER JOIN appweb-PorlamarAJ.dbo.Usuarios AS E ON CAST(E.cedula AS BIGINT) = B.usuario
