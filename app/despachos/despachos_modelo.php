...
class Despachos extends Conectar
{
    ...
    public function getExisteDocumentoEnDespachos($numerod, $tipodoc)
    {
        ...
        //QUERY
        $sql =
            'SELECT Numerod FROM Despachos_Det WHERE Numerod = ? AND Tipofac = ?';
    ...
    public function getDocumentoEnDespachos(
        $numerod,
        $tipodoc,
        ...
        $activo = $evaluate_active ? " AND Activo = '1'" : '';
    ...
    public function getProductosDespachoCreadoEnFacturas($correlativo)
    {
        ...
        //QUERY
        $sql =
            "SELECT DISTINCT coditem, descrip, cantempaq, esempaque, saprod.Tara as tara, codinst,
                    SUM(CASE WHEN EsUnid=0 THEN cantidad ELSE 0 END) AS bultos,
                    SUM(CASE WHEN EsUnid=1 THEN cantidad ELSE 0 END) AS paquetes
                FROM SAITEMFAC
                    INNER JOIN SAPROD ON SAITEMFAC.CodItem = SAPROD.CodProd
                WHERE TipoFac = 'A' AND (numerod in ( SELECT Numerod FROM [" .
            NAME_BD_1 .
            "].dbo.Despachos_Det WHERE ID_Correlativo = ? AND TipoFac = 'A'))
    ...
    public function getProductosDespachoCreadoEnNotaDeEntrega($correlativo)
    {
        ...
        //QUERY
        $sql =
            "SELECT coditem, descrip, cantempaq, esempaque, saprod.Tara as tara, codinst,
                ...
                WHERE SAITEMNOTA.TipoFac = 'C' AND (numerod in (SELECT Numerod FROM [" .
            NAME_BD_1 .
            "].dbo.Despachos_Det WHERE ID_Correlativo = ? AND TipoFac = 'C'))
    ...
    public function getCabeceraDespacho($correlativo)
    {
        ...
        //QUERY
        $sql =
            'SELECT Correlativo, fechae, fechad, ID_Chofer, ID_Vehiculo, Destino, ID_Usuario FROM Despachos where Correlativo = ?';
    ...
    public function insertarDespacho($values)
    {
        ...
        //QUERY
        $sql =
            'INSERT INTO Despachos (fechae, fechad, ID_Chofer, ID_Vehiculo, Destino, ID_Usuario) VALUES (?, ?, ?, ?, ?, ?)';
    ...
    public function insertarDetalleDespacho(
        $correlativo,
        $numero_documento,
        $tipo_documento,
        $estado = null
        ...
        if (is_null($estado)) {
            $sql =
                'INSERT INTO Despachos_Det (ID_Correlativo, Numerod, Tipofac) VALUES (?, ?, ?)';
        } else {
            $sql =
                'INSERT INTO Despachos_Det (ID_Correlativo, Numerod, Estado, Tipofac) VALUES (?, ?, ?, ?)';
        }
    ...
    public function updateDespacho(
        ...
        //QUERY
        $sql =
            'UPDATE Despachos SET Destino = ?,  ID_Chofer = ?,  ID_Vehiculo = ?,  fechad = ? WHERE Correlativo = ?';
    ...
    public function updateDetalleDespacho(
        $correlativo,
        $numerod_nuevo,
        $numerod_viejo,
        $tipofac_nuevo,
        ...
        //QUERY
        $sql =
            'UPDATE Despachos_det SET Numerod = ?, Tipofac = ? WHERE ID_Correlativo = ? AND Numerod = ? AND Tipofac = ?';
    ...
    public function deleteDetalleDespacho($correlativo, $documento, $tipofac)
    {
        ...
        //QUERY
        $sql =
            'DELETE FROM Despachos_Det WHERE ID_Correlativo = ? AND Numerod = ? AND Tipofac = ?';
    ...
    public function getMercanciaSinDespachar($fechai, $fechaf, $alm = [])
    {
        $i = 0;
        $cond = $depo = '';
        //LLAMAMOS A LA CONEXION QUE CORRESPONDA CUANDO ES SAINT: CONEXION2
        ...
        //QUERY
        $sql =
            "SELECT CodProd, Descrip, CantEmpaq,
                    (SELECT SUM(cantidad) FROM SAITEMFAC WHERE esunid='0' AND CodProd=CodItem " .
            $cond .
            " AND numerod IN (SELECT fa.numerod FROM SAFACT AS fa WHERE TipoFac='A' AND
                    DATEADD(dd, 0, DATEDIFF(dd, 0, FechaE)) BETWEEN ? AND ? AND (NumeroR IS NULL OR NumeroD IN (SELECT x.NumeroR FROM SAFACT AS x WHERE x.TipoFac = 'B' AND x.NumeroR=fa.NumeroD AND DATEADD(dd, 0, DATEDIFF(dd, 0, FechaE)) BETWEEN ? AND ? GROUP BY x.NumeroR HAVING cast(SUM(x.Monto) AS BIGINT)<cast(fa.Monto AS BIGINT))) AND NumeroD NOT IN (SELECT Despachos_Det.Numerod FROM APPWEB_PORLAMAR.dbo.Despachos_Det)))
                    AS todob,
                    (SELECT SUM(cantidad) FROM SAITEMFAC WHERE esunid='1' AND CodProd=CodItem " .
            $cond .
            " AND numerod IN (SELECT fa.numerod FROM SAFACT AS fa WHERE TipoFac='A' AND
                    DATEADD(dd, 0, DATEDIFF(dd, 0, FechaE)) BETWEEN ? AND ? AND (NumeroR IS NULL OR NumeroD IN (SELECT x.NumeroR FROM SAFACT AS x WHERE x.TipoFac = 'B' AND x.NumeroR=fa.NumeroD AND DATEADD(dd, 0, DATEDIFF(dd, 0, FechaE)) BETWEEN ? AND ? GROUP BY x.NumeroR HAVING cast(SUM(x.Monto) AS BIGINT)<cast(fa.Monto AS BIGINT))) AND NumeroD NOT IN (SELECT Despachos_Det.Numerod FROM APPWEB_PORLAMAR.dbo.Despachos_Det)))
                    AS todop,
                    (SELECT SUM(Existen) FROM SAEXIS WHERE CodProd=SAPROD.CodProd " .
            $cond .
            ")
                    AS exis,
                    (SELECT SUM(ExUnidad) FROM SAEXIS WHERE CodProd=SAPROD.CodProd  " .
            $cond .
            ")
                    AS exunid
                FROM SAPROD WHERE CodProd IN (SELECT coditem FROM SAITEMFAC WHERE TipoFac='A' " .
            $cond .
            "
