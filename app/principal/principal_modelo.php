       ...
       /* $sql = "SELECT SUM(total-abono) as saldo_dolares
        ...
        $sql = "SELECT 
          SUM(SAACXC.Saldo) as saldo_dolares
           from MCONFISUR_D.dbo.saacxc inner join MCONFISUR_D.dbo.saclie on saacxc.codclie = saclie.codclie 
           where saacxc.saldo>0 AND (saacxc.tipocxc='10' OR saacxc.tipocxc='20')" ;
 ...
 ROUND((SUM(((CostAct/NULLIF(CantEmpaq,0))))), 2) as suma,CantEmpaq as display ,SUM(exis.Existen) AS cantidad_b, SUM(exis.exunidad) AS cantidad_p ,CONVERT(FLOAT,CostAct) AS costo
    ...
    public function get_devoluciones_sin_motivo_Factura($tipodespacho) {
        ...
        $relation = hash_equals('1', $tipodespacho)
            ? ' inner join appweb-PorlamarAJ.dbo.Despachos_Det desp on desp.numerod = NumeroR '
            : '';

        $condition = hash_equals('0', $tipodespacho)
            ? " AND (numerod NOT IN (SELECT numerod FROM appweb-PorlamarAJ.dbo.Despachos_Det) AND NumeroR NOT IN (SELECT numerod FROM appweb-PorlamarAJ.dbo.Despachos_Det)) "
    ...
    public function get_devoluciones_sin_motivo_NotadeEntrega($tipodespacho) {
        ...
        $relation = hash_equals('1', $tipodespacho)
            ? ' INNER JOIN appweb-PorlamarAJ.dbo.Despachos_Det AS desp ON desp.numerod = numerof'
            : '';

        $condition = hash_equals('0', $tipodespacho)
            ? " AND (numerod NOT IN (SELECT numerod FROM appweb-PorlamarAJ.dbo.Despachos_Det) AND numerof NOT IN (select numerod FROM appweb-PorlamarAJ.dbo.Despachos_Det))"
