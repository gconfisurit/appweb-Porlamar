...
class Principal extends Conectar
{
    public function getDocumentosSinDespachar()
    {
        ...
        $sql = "SELECT *
                ...
                WHERE SA.NumeroD NOT IN (SELECT Despachos_Det.Numerod FROM APPWEB_PORLAMAR.dbo.Despachos_Det)
                  AND SA.TipoFac = 'A'
    ...
    public function get_clientes_por_tipo($tipo = 0)
    {
        ...
        //QUERY
        $sql =
            "SELECT codclie, descrip, id3, codvend, fechae, tipoid3, activo FROM SACLIE WHERE activo ='1' AND TipoID3 = ? AND codvend NOT IN ('99')";
    ...
    public function get_tasa_dolar()
    {
        ...
        //QUERY
        $sql =
            'SELECT TOP(1) FechaE AS fechae, Tasa AS tasa FROM SACOMP WHERE Tasa IS NOT NULL ORDER BY FechaE DESC';
    ...
    public function get_devoluciones_sin_motivo_Factura($tipodespacho)
    {
        ...
        $relation = hash_equals('1', $tipodespacho)
            ? ' inner join APPWEB_PORLAMAR.dbo.Despachos_Det desp on desp.numerod = NumeroR '
            : '';

        $condition = hash_equals('0', $tipodespacho)
            ? ' AND (numerod NOT IN (SELECT numerod FROM APPWEB_PORLAMAR.dbo.Despachos_Det) AND NumeroR NOT IN (SELECT numerod FROM APPWEB_PORLAMAR.dbo.Despachos_Det)) '
    ...
    public function get_devoluciones_sin_motivo_NotadeEntrega($tipodespacho)
    {
        ...
        $relation = hash_equals('1', $tipodespacho)
            ? ' INNER JOIN APPWEB_PORLAMAR.dbo.Despachos_Det AS desp ON desp.numerod = numerof'
            : '';

        $condition = hash_equals('0', $tipodespacho)
            ? ' AND (numerod NOT IN (SELECT numerod FROM APPWEB_PORLAMAR.dbo.Despachos_Det) AND numerof NOT IN (select numerod FROM APPWEB_PORLAMAR.dbo.Despachos_Det))'
