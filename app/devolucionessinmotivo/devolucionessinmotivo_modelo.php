...
class Devolucionessinmotivo extends Conectar
{
    ...
    public function getDocumentoEnDespacho($data)
    {
        ...
        //QUERY
        $sql =
            'SELECT numerod FROM Despachos_Det WHERE Numerod = ? or Numerod = ?';
    ...
    public function editar_motivo($data)
    {
        ...
        $sql =
            'UPDATE Despachos_Det SET  Observacion=? WHERE Numerod = ? or Numerod = ?';
    ...
    public function insertar_motivo($data)
    {
        ...
        $usuario_id =
            (is_array($_SESSION) == true and count($_SESSION) > 0)
                ? $_SESSION['cedula']
                : '';

        $sql =
            'INSERT INTO Despachos_Det (Numerod, Observacion, Nnotacre, Fecha_Liqui, Fecha_Entre, ID_Usuario, Tipofac) VALUES (?,?,?,?,?,?,?)';
