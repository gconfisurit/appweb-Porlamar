    ...
    public function getcobros($fechai, $fechaf, $ruta, $tipo)
    {
        ...
        if ($tipo === 'B' and $ruta === 'Todos') {
            $sql = "SELECT EDV, 
              ...
              p.Monto - isnull((select dev.Monto from [MCONFISUR].[dbo].SAACXC as dev where dev.NroUnico = p.NroPpal and dev.TipoCxc in ('10','20')),0) 
              ...
              p.Monto - isnull((select dev.Monto from [MCONFISUR].[dbo].SAACXC as dev where dev.NroUnico = p.NroPpal and dev.TipoCxc in ('10','20')),0) Total
              
              from [MCONFISUR].[dbo].SAPAGCXC as p 
              inner join [MCONFISUR].[dbo].SAACXC as c on p.NroPpal = c.NroUnico 
              inner join [MCONFISUR].[dbo].SACLIE as cl on c.CodClie = cl.CodClie
              left join [MCONFISUR].[dbo].SAACXC as cxc on cxc.NumeroD = p.NumeroD and cxc.TipoCxc in ('10','20')
            ...
            if ($tipo === 'D' and $ruta === 'Todos') {
                $sql = "SELECT EDV, 
                    ...
                    c.CodVend EDV,
                    case when DATEDIFF(day, cxc.FechaE, p.FechaE) >= 0 and DATEDIFF(day, cxc.FechaE, p.FechaE) <= 7 then
                    p.Monto - isnull((select dev.Monto from [MCONFISUR_D].[dbo].SAACXC as dev where dev.NroUnico = p.NroPpal and dev.TipoCxc in ('10','20')),0) 
                    else 0 end De_0_a_7_Dias,
                    ...
                    p.Monto - isnull((select dev.Monto from [MCONFISUR_D].[dbo].SAACXC as dev where dev.NroUnico = p.NroPpal and dev.TipoCxc in ('10','20')),0) Total
                    
                    from [MCONFISUR_D].[dbo].SAPAGCXC as p 
