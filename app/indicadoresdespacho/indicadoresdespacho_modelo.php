...
{
    public function get_entregasefectivas_por_chofer(
        $fechai,
        $fechaf,
        $id_chofer
    ) {
    ...
    public function get_causasrechazo_por_chofer(
        $fechai,
        $fechaf,
        $id_chofer,
        $crechaz
        ...
        $sql = "SELECT det.correl AS correlativo, fecha_entre, count(det.correl) AS cant_documentos, tipo_pago, observacion, color, rechazo.id AS color_id
               ...
               LEFT JOIN APPWEB_PORLAMAR.dbo.M_rechazos AS rechazo ON UPPER(descripcion) = UPPER(det.observacion) 
    ...
    public function get_oportunidaddespacho_por_chofer(
        $fechai,
        $fechaf,
        $id_chofer
    ) {
    ...
    public function get_periodos($indicador, $tipo, $id_chofer)
    {
        ...
        switch ($tipo) {
            ...
            case 'Mensual':
                $month_in_parameter =
                    ', MONTH(CAST(fecha_entre AS DATETIME)) mes ';
                $month_in_condition =
                    ' AND DATEADD(dd, 0, DATEDIFF(dd, 0, fechad)) BETWEEN ? AND ?  ';
        ...
        switch ($indicador) {
            ...
            case 2:
                $condition_indicador =
                    " AND observacion IS NOT NULL AND observacion != '' ";
                break;
            case 3:
                $condition_indicador = ' AND SAFACT.TipoFac IN (?,?) ';
                break;
        }
