...
switch ($_GET['op']) {
    case 'listar_libroventas':
        ...
        $retenciones_otros_periodos = $libroventa->getRetencionesOtrosPeriodos(
            $fechai,
            $fechaf
        );

        //DECLARAMOS UN ARRAY PARA EL RESULTADO DEL MODELO.
        $data = $totales_libro = $resumen = $otros_periodos = $totales_otros_periodos = [];
        ...
        if (is_array($datos) == true and count($datos) > 0) {
            foreach ($datos as $key => $row) {
                $retencion_dato = $libroventa->getRetencionItem(
                    $fechai,
                    $fechaf,
                    $row['numerodoc']
                );
                ...
                $totalventasconiva =
                    $base_imponible +
                    $row['mtoexento'] +
                    $row['MontoIVA_ContribuyeIVA14'];

                $tvii += $totalventasconiva;
                $ve += $row['mtoexento'];
                $magbi16c += $base_imponible;
                $mag16c += $row['MontoIVA_ContribuyeIVA14'];
                $ivare +=
                    count($retencion_dato) > 0
                        ? Numbers::avoidNull($retencion_dato[0]['retencioniva'])
                        : 0;

                $sub_array['num'] = $key + 1;
                $sub_array['fechaemision'] = date(
                    FORMAT_DATE,
                ...
                $sub_array['factafectada'] = Strings::avoidNull(
                    $row['factafectada']
                );
                $sub_array['nroretencion'] =
                    count($retencion_dato) > 0
                        ? Strings::avoidNull($retencion_dato[0]['nroretencion'])
                        : '';
                $sub_array['totalventasconiva'] = Strings::rdecimal(
                    $totalventasconiva,
                    2
                );
                $sub_array['mtoexento'] = Strings::rdecimal(
                    $row['mtoexento'],
                    2
                );
                $sub_array['base_imponible'] = Strings::rdecimal(
                    $base_imponible,
                    2
                );
                $sub_array['alicuota_contribuyeiva'] = Strings::rdecimal(
                    $row['Alicuota_ContribuyeIVA14'],
                    0
                );
                $sub_array['MontoIVA_ContribuyeIVA14'] = Strings::rdecimal(
                    $row['MontoIVA_ContribuyeIVA14'],
                    2
                );
                $sub_array['retencioniva'] =
                    count($retencion_dato) > 0
                        ? Strings::avoidNull($retencion_dato[0]['retencioniva'])
                        : '';
        ...
        if (
            is_array($retenciones_otros_periodos) == true and
            count($retenciones_otros_periodos) > 0
        ) {
            foreach ($retenciones_otros_periodos as $key => $row) {
                ...
                $sub_array['fecharetencion'] = !is_null($row['fecharetencion'])
                    ? date(FORMAT_DATE, strtotime($row['fecharetencion']))
                    : '';
                $sub_array['totalgravable_contribuye'] = Strings::rdecimal(
                    $row['totalgravable_contribuye'],
                    2
                );
                $sub_array['totalivacontribuye'] = Strings::rdecimal(
                    $row['totalivacontribuye'],
                    2
                );
                $sub_array['retencioniva'] = Strings::rdecimal(
                    $row['retencioniva'],
                    2
                );
        ...
        $resumen = [
            ...
            [
                'descripcion' =>
                    'Total ventas internas Gravadas por Alicuota General (16%)',
            ...
            [
                'descripcion' =>
                    'Total ventas internas Gravadas por Alicuota General',
                'base_imponible' => Strings::rdecimal(0, 2),
                'credito_fiscal' => Strings::rdecimal(0, 2),
                'isBold' => false,
                'isColored' => false,
